...
func TestPasslib(t *testing.T) {
	for _, testScheme := range DefaultSchemes {
		//t.Logf("scheme: %+v\n", scheme)
		c := Context{Schemes: []scheme.Scheme{testScheme}}
...
func kat(t *testing.T, testScheme scheme.Scheme, password, hash string) {
	c := Context{Schemes: []scheme.Scheme{testScheme}}

	_, err := c.Verify(password, hash)

	if err != nil {
		t.Logf("err verifying known good hash: %v %s %s", testScheme, password, hash)
		t.Fail()
	}

	_, err = c.Verify(" "+password, hash)
	if err == nil {
		t.Logf("invalid verification of known hash: %v", testScheme)
		t.Fail()
	}
}
...
func TestKat(t *testing.T) {
	for _, v := range []struct{ p, h string }{
		...
		kat(t, sha2.Crypter256, v.p, v.h) //"foobar", "$5$rounds=110000$J672cUm182wrK1bX$0TzjpY6NV07r82J9YebG50dZuwHoQWrny9Q7y6ceO7/")
	}
